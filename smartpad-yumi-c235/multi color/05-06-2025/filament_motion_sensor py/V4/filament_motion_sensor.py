...
class EncoderSensor:
    ...
    def encoder_event(self, eventtime, state):
        if self.extruder is not None:
            ...
            if self.last_event_pos is not None and (self.pitch_view or self.blockage_detection):
                distance = abs(current_pos - self.last_event_pos)
                time_diff = eventtime - self.last_event_time
                
                # Avoid division by zero and calculate pitch
                if time_diff > 0:
                    pitch = distance / time_diff
                    
                    # Display pitch in console if enabled
                    if self.pitch_view:
                        logging.info("Filament pitch: %.2f mm/s", pitch)
                    ...
                    if self.blockage_detection:
                        if pitch < self.min_pitch or pitch > self.max_pitch:
                            ...
                            if self.anomaly_count >= self.blockage_threshold:
                                logging.error("Filament blockage detected: pitch=%.2f mm/s", pitch)
                                # Optional: Add custom action here (e.g. pause print)
                        else:
                            self.anomaly_count = 0
            
            # Update last event position/time
