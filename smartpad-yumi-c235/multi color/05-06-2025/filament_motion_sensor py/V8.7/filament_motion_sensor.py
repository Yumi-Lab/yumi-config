# Filament Motion Sensor Module - Version Finale
#
# Copyright (C) 2021 Joshua Wherrett <thejoshw.code@gmail.com>
# Modifications : Compteurs de seuil corrigés
#
...
CHECK_RUNOUT_TIMEOUT = .250  # 250ms pour meilleure réactivité
...
class EncoderSensor:
    def __init__(self, config):
        ...
        if self.blockage_detection:
            ...
            self.in_reset_phase = False  # Nouvel état pour gérer la phase de reset
    ...
    def _log_data(self, eventtime, sensor_name, position, delta, state, status):
        ...
        try:
            ...
            writer.writerow([
                ...
                status,
                f"{self.anomaly_count}/{self.blockage_threshold}" if self.blockage_detection else "N/A",
                f"{self.normal_count}/{self.reset_threshold}" if self.blockage_detection else "N/A"
            ])
            self.log_file.flush()
        except Exception as e:
    ...
    def _process_movement(self, eventtime, delta, sensor_name):
        ...
        if not self.blockage_detection:
            if self.pitch_view:
                msg = "%s - Pitch: %.3f mm [NORMAL]" % (sensor_name, abs_delta)
                self.printer.lookup_object('gcode').respond_info(msg)
            return status
        ...
        if abs_delta < self.min_pitch or abs_delta > self.max_pitch:
            ...
            # Si on était en phase de reset, on recommence à zéro
            ...
            # Si blocage précédent, on démarre la phase de reset
            if self.triggered:
                self.triggered = False
                self.in_reset_phase = True
                self.normal_count = 1
            # Si en phase de reset, on incrémente
            elif self.in_reset_phase:
                self.normal_count += 1
            ...
            # Sinon, comportement normal
                ...
                self.normal_count = 0  # Reset du compteur normal si pas d'anomalie récente

        # Affichage
        if self.pitch_view:
            counter_info = ""
            if status == "ABNORMAL":
                counter_info = f" [{self.anomaly_count}/{self.blockage_threshold}]"
            elif status == "NORMAL" and self.in_reset_phase:
                counter_info = f" [{self.normal_count}/{self.reset_threshold}]"
