...
# Modifications: Data logging and low pitch filtering
#
...
class EncoderSensor:
    ...
    def _init_data_logging(self):
        ...
        try:
            ...
            logging.info("Data logging enabled for %s to %s",
    ...
    def _log_data(self, eventtime, sensor_name, position, delta, state, filtered):
    ...
    def encoder_event(self, eventtime, state):
        ...
        if self.last_event_pos is not None:
            ...
            if self.low_pitch_filter > 0 and abs_delta <= self.low_pitch_filter:
                filtered = True
                logging.debug("%s - Low pitch filtered: %.5f mm", sensor_name, abs_delta)
            else:
                # Only process positive movements (extrusion)
                if delta > 0:
                    if self.pitch_view or self.blockage_detection:
                        self._process_movement(eventtime, delta, sensor_name, filtered)
                elif delta < 0:
        ...
        # Log data if enabled
        if self.data_logging:
            delta_log = current_pos - self.last_event_pos if self.last_event_pos is not None else 0
            self._log_data(eventtime, sensor_name, current_pos, delta_log, state, filtered)
    ...
    def _process_movement(self, eventtime, delta, sensor_name, filtered):
        abs_delta = abs(delta)
        
        if filtered:
            return  # Skip processing if filtered
