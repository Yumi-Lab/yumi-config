...
class PrinterProbePressure:
    def __init__(self, config, mcu_probe):
        ...
        self.printer.lookup_object('pins').register_chip('probe_pressure', self)
        ...
        self.gcode = self.printer.lookup_object('gcode')
        self.gcode.register_command('PROBE_PRESSURE1', self.cmd_PROBE,
                                    desc=self.cmd_PROBE_help)
        self.gcode.register_command('QUERY_PROBE1', self.cmd_QUERY_PROBE,
                                    desc=self.cmd_QUERY_PROBE_help)
        self.gcode.register_command('PROBE_CALIBRATE1', self.cmd_PROBE_CALIBRATE,
                                    desc=self.cmd_PROBE_CALIBRATE_help)
        self.gcode.register_command('PROBE_ACCURACY1', self.cmd_PROBE_ACCURACY,
                                    desc=self.cmd_PROBE_ACCURACY_help)
        self.gcode.register_command('Z_OFFSET_APPLY_PROBE1',
    ...
    def _probe(self, speed):
        ...
        if 'z' not in toolhead.get_status(curtime)['homed_axes']:
            self.gcode.run_script_from_command('M117 Tip code: 124')
...
class ProbePointsHelper:
    ...
    def start_probe(self, gcmd):
        manual_probe.verify_no_manual_probe(self.printer)
        # Lookup objects
        probe = self.printer.lookup_object('probe_pressure', None)
...
def load_config(config):
    probePressure = PrinterProbePressure(config, ProbeEndstopWrapper(config))
    config.get_printer().add_object('probe_pressure', probePressure)
    return probePressure
